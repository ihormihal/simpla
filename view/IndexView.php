...
{	
	...
	function fetch()
	{
		...
		foreach ($places as $place) {
			$phones = array();
			$phone = json_decode($place->phone, true);
			foreach ($phone as $p) {
				if($p){
					preg_match('/\(.*\)/', $p, $code);
					$phones[] = array('code' => $code[0], 'number' => str_replace($code[0],'',$p));
				}
			}
			$place->phones = $phones;
		...
		//если хочем пустой шаблон
		if($this->request->get('wrapper', 'string') == 'empty'){
			$wrapper = '';
		}

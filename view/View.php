...
class View extends Simpla
{
	...
	public function __construct()
	{
		...
		else
		{
			...
			$this->design->smarty->registerPlugin("function", "get_product",             array($this, 'get_product_plugin'));
		}
	}
	...
	public function get_product_plugin($params, &$smarty)
	{
		...
		if(isset($params['id'])){
			...
			$variants = array();
			foreach($this->variants->get_variants(array('product_id'=>$product_id, 'in_stock'=>true)) as $v){
				$variants[$v->id] = $v;
			}
			$images = $this->products->get_images(array('product_id'=>$product_id));

...
class CartView extends View
{
  ...
  public function __construct()
  {
    ...
    if(isset($_POST['checkout']) || isset($_POST['fast_checkout']))
    {
    	$order = new stdClass;
		...
		if(!$order->delivery_id){
			$order->delivery_id = $this->settings->default_delivery;
		}
		...
		$order->cart_discount = $cart->cart_discount;
	    ...
	    if(isset($_POST['fast_checkout'])){
	    	...
	    	if(empty($order->delivery_id)) $order->delivery_id = $this->settings->default_delivery;
	    	if(empty($order->payment_method_id)) $order->payment_method_id = $this->settings->default_payment;
	    	if(empty($order->phone)) $this->design->assign('error', 'empty_phone');	
	    	if(empty($order->email)) $order->email = '-';
		    if(empty($order->name)) $order->name = 'Быстрый заказ';
		    if(empty($order->address)) $order->address = '-';
		    if(empty($order->comment)) $order->comment = '-';
	    }

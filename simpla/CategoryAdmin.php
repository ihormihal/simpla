...
class CategoryAdmin extends Simpla
{
  ...
  function fetch()
  {
		...
		if($this->request->method('post'))
		{
			...
			$category->product_id = $this->request->post('product_id', 'integer');
		...
		$filter = array('category_id'=>$category->children);
		$category_products = $this->products->get_products($filter);
		$this->design->assign('category_products', $category_products);

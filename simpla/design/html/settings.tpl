{capture name=tabs}
	...
	{if in_array('settings', $manager->permissions)}<li><a href="{url module=ContactsAdmin}">Контакты</a></li>{/if}
{/capture}
...
<input type=hidden name="session_id" value="{$smarty.session.id}">
		...
		<div class="block layer">
			...
			<ul>
				<li>
					<label class=property>Тип доставки по-умолчанию</label>
					<select class="simpla_inp" name="default_delivery">
						<option value="">--</option>
						{foreach $deliveries as $method}
						<option value="{$method->id}" {if $settings->default_delivery == $method->id}selected{/if}>{$method->name}</option>
						{/foreach}
					</select>
				</li>
				<li>
					<label class=property>Тип оплаты по-умолчанию</label>
					<select class="simpla_inp" name="default_payment">
						<option value="">--</option>
						{foreach $payment_methods as $method}
						<option value="{$method->id}" {if $settings->default_payment == $method->id}selected{/if}>{$method->name}</option>
						{/foreach}
					</select>
				</li>
			</ul>
		...
		<!-- Параметры -->
		<div class="block layer">
			<h2>Скидка при покупке через корзину</h2>
			<ul>
				<li>
					<label class=property>Скидка для всех товаров</label>
					<input name="cart_discount" class="simpla_inp" type="text" value="{$settings->cart_discount|escape}" /> %
				</li>
			</ul>
		</div>
		<!-- Параметры (The End)-->

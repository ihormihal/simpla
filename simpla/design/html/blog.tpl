...
{capture name=tabs}
	...
	<li><a href="index.php?module=BlogCategoriesAdmin">Категории блога</a></li>
{/capture}
...
<div id="header">
	...
	{elseif $posts_count}
	<h1>{$posts_count} {$posts_count|plural:'запись':'записей':'записи'} {if $cat}в <span>"{$cat->name}"</span>{else}в блоге{/if}</h1>
	{else}
	<h1>Нет записей</h1>
	{/if}
...
<div id="right_menu">
	<ul>
		<li {if !$cat}class="selected"{/if}><a href="index.php?module=BlogAdmin"><i class="fa fa-angle-right"></i>Все новости</a></li>

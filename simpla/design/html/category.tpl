...
<input type=hidden name="session_id" value="{$smarty.session.id}">
	...
	<div id="column_right">
		...
		<!-- Товар категории --> 	
		<div class="block layer">
			<h2>Товар категории</h2>
			<select name="product_id" style="width:100%;padding:0.25rem;">
				<option value="">Выберите главный товар</option>
				{foreach $category_products as $product}
					<option value="{$product->id}" {if $category->product_id == $product->id}selected{/if}>{$product->name}</option>
				{/foreach}	
			</select>
		</div>

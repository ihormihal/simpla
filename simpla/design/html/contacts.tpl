...
{* Title *}
{$meta_title='Контакты' scope=parent}

{* Заголовок *}
<div id="header">
	<h1>Контакты</h1> 
	<a class="add" href="{url module=ContactAdmin}">Добавить магазин</a>
</div>	
...
<div id="main_list">
	...
	<div id="list">
		...
		<div class="{if !$place->enabled}invisible{/if} row">
			...
			<div class="name cell">		
				<a href="{url module=ContactAdmin id=$place->id return=$smarty.server.REQUEST_URI}">{$place->city|escape}</a>
			</div>
...
$(function() {
	...
	$("a.enable").click(function() {
		...
		$.ajax({
			...
			data: {'object': 'place', 'id': id, 'values': {'enabled': state}, 'session_id': '{/literal}{$smarty.session.id}{literal}'},

...
<input type=hidden name="session_id" value="{$smarty.session.id}">
	...
	<div id="variants_block" {assign var=first_variant value=$product_variants|@first}{if $product_variants|@count <= 1 && !$first_variant->name}class=single_variant{/if}>
		<ul id="header">
			...
			<li class="variant_custom">Под заказ</li>
		</ul>
		<div id="variants">
		...
		<ul>
			...
			<li class="variant_custom">
				<select name="variants[custom][]">
					<option value="0">--</option>
					<option value="1" {if $variant->custom}selected{/if}>Да</option>
				</select>
			</li>
		...
		<ul id=new_variant style='display:none;'>
			...
			<li class="variant_custom">    <input name="variants[custom][]" type="checkbox" {if $variant->custom}value="1" checked{else}value="0"{/if}/></li>
			<li class="variant_download">

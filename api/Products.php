...
class Products extends Simpla
{
	...
	{
		$query = $this->db->placehold('SELECT MIN(price) as minprice FROM __variants WHERE (stock IS NULL OR stock>0) AND price>0 AND product_id in (SELECT product_id FROM __products_categories WHERE category_id in(?@))', (array)$category_id);  
		$this->db->query($query);
		return $this->db->result('minprice');
	}

	public function getMaxPrice($category_id)
	{
		$query = $this->db->placehold('SELECT MAX(price) as maxprice FROM __variants WHERE (stock IS NULL OR stock>0) AND product_id in (SELECT product_id FROM __products_categories WHERE category_id in(?@))', (array)$category_id);  
		$this->db->query($query);
		return $this->db->result('maxprice');
	}

	public function getMinPriceBrand($brand_id)
	{
		$query = $this->db->placehold('SELECT MIN(price) as minprice FROM __variants WHERE (stock IS NULL OR stock>0) AND price>0 AND product_id in (SELECT id FROM __products WHERE brand_id = ?)', (int)$brand_id);  
		$this->db->query($query);
		return $this->db->result('minprice');
	}

	public function getMaxPriceBrand($brand_id)
	{
		$query = $this->db->placehold('SELECT MAX(price) as maxprice FROM __variants WHERE (stock IS NULL OR stock>0) AND product_id in (SELECT id FROM __products WHERE brand_id = ?)', (int)$brand_id);  
		$this->db->query($query);
		return $this->db->result('maxprice');
	}
	/**
	...
	*/
	...
	{		
		...
		if($filter['maxCost'])
			$price_filter .= $this->db->placehold('AND p.id in (SELECT product_id FROM __variants WHERE price >= ? AND price <= ?)', $filter['minCost'], $filter['maxCost']);
	...
	*/
	...
	{		
		...
		if(!empty($filter['minCost']) && !empty($filter['maxCost']))
			$price_filter .= $this->db->placehold('AND p.id in (SELECT product_id FROM __variants WHERE price >= ? AND price <= ?)', $filter['minCost'], $filter['maxCost']);

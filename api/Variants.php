...
class Variants extends Simpla
{
	...
	*/
	...
	{		
		...
		$query = $this->db->placehold("SELECT v.id, v.product_id , v.price, NULLIF(v.compare_price, 0) as compare_price, v.sku, IFNULL(v.stock, ?) as stock, (v.stock IS NULL) as infinity, v.custom, v.name, v.attachment, v.position
		...
		foreach ($variants as $v) {
			$v->cart_discount = $v->price*($this->settings->cart_discount)/100;
		}
		return $variants;
	}
	...
	{	
		...
		$query = $this->db->placehold("SELECT v.id, v.product_id , v.price, NULLIF(v.compare_price, 0) as compare_price, v.sku, IFNULL(v.stock, ?) as stock, (v.stock IS NULL) as infinity, v.custom, v.name, v.attachment
